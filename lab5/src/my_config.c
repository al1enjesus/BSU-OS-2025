#include <linux/module.h>
#include <linux/kernel.h>
#include <linux/init.h>
#include <linux/proc_fs.h>
#include <linux/uaccess.h>
#include <linux/string.h>

#define PROC_NAME "my_config"
#define BUF_SIZE 256

static struct proc_dir_entry *proc_entry;

static char config_buf[BUF_SIZE] = "default";
static size_t config_len = 7;   // strlen("default")

static ssize_t my_config_read(struct file *file, char __user *ubuf,
                              size_t count, loff_t *ppos)
{
    char kbuf[BUF_SIZE + 2];
    int len;

    if (*ppos > 0)
        return 0;

    len = scnprintf(kbuf, sizeof(kbuf), "%s\n", config_buf);

    if (len > count)
        len = count;

    if (copy_to_user(ubuf, kbuf, len))
        return -EFAULT;

    *ppos = len;
    return len;
}

static ssize_t my_config_write(struct file *file, const char __user *ubuf,
                               size_t count, loff_t *ppos)
{
    size_t to_copy;

    if (count == 0)
        return 0;

    if (count >= BUF_SIZE)
        to_copy = BUF_SIZE - 1;
    else
        to_copy = count;

    if (copy_from_user(config_buf, ubuf, to_copy))
        return -EFAULT;

    config_buf[to_copy] = '\0';
    config_len = to_copy;
    *ppos = 0;

    printk(KERN_INFO "my_config: new value = '%s'\n", config_buf);
    return count;
}

static const struct proc_ops my_config_ops = {
    .proc_read  = my_config_read,
    .proc_write = my_config_write,
};

static int __init my_config_init(void)
{
    proc_entry = proc_create(PROC_NAME, 0666, NULL, &my_config_ops);
    if (!proc_entry) {
        printk(KERN_ERR "my_config: failed to create /proc/%s\n", PROC_NAME);
        return -ENOMEM;
    }

    printk(KERN_INFO "my_config: module loaded, default='%s'\n", config_buf);
    return 0;
}

static void __exit my_config_exit(void)
{
    if (proc_entry)
        proc_remove(proc_entry);

    printk(KERN_INFO "my_config: module unloaded\n");
}

module_init(my_config_init);
module_exit(my_config_exit);

MODULE_LICENSE("GPL");
MODULE_AUTHOR("Koscov Artem");
MODULE_DESCRIPTION("Lab5: /proc/my_config with read/write");
MODULE_VERSION("1.0");
