...
// Используем своё имя, чтобы не конфликтовать с системным mode_t
typedef enum { MODE_UNSYNC, MODE_MUTEX, MODE_ATOMIC } run_mode_t;
...
static atomic_llong shared_counter_atomic; // инициализируется в main
...
static run_mode_t parse_mode(const char* s) {
...
int main(int argc, char** argv) {
    ...
    run_mode_t mode = parse_mode(argv[3]);
    ...
    // Явная инициализация всех счётчиков
