...
int main(int argc, char* argv[]) {
    if (argc != 4) {
        fprintf(stderr, "Usage: %s <threads> <increments> <unsync|mutex|atomic>\n", argv[0]);
        return EXIT_FAILURE;
    }
    ...
    printf("Threads: %d, Increments: %lld\n", n_threads, per_thread);
    printf("Expected: %lld, Observed: %lld\n", expected, observed);
    printf("Elapsed: %.6f seconds\n", elapsed);
    printf("Result: %s\n", (expected == observed) ? "CORRECT" : "INCORRECT");
