...
static void *worker_unsync(void *arg) {
  ...
  // TODO: реализуйте небезопасное инкрементирование общего счетчика без
  // синхронизации Пример цели: shared_counter_unsync++ внутри цикла для
  // демонстрации гонки
...
static void *worker_atomic(void *arg) {
  ...
  for (long long i = 0; i < a->iterations_per_thread; i++) {
    // TODO: atomic_fetch_add_explicit(&shared_counter_atomic, 1,
...
int main(int argc, char **argv) {
  if (argc < 4) {
    fprintf(stderr,
            "Usage: %s <num_threads> <iterations_per_thread> "
            "<unsync|mutex|atomic>\n",
            argv[0]);
    return 1;
  }
  ...
  thread_args_t *args =
      (thread_args_t *)calloc((size_t)num_threads, sizeof(thread_args_t));
  if (!tids || !args) {
  ...
  fprintf(stderr, "[thread_race] Внимание: это скелет (samples). Реализуйте "
                  "TODO для осмысленных результатов.\n");
  ...
  if (mode == MODE_UNSYNC)
    ...
    actual = atomic_load_explicit(&shared_counter_atomic, memory_order_relaxed);

  printf("mode=%s threads=%d iters_per_thread=%lld expected=%lld actual=%lld "
         "time_ms=%lld\n",
         (mode == MODE_UNSYNC  ? "unsync"
          : mode == MODE_MUTEX ? "mutex"
                               : "atomic"),
