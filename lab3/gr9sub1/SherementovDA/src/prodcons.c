...
static void* consumer_thread(void* arg) {
    ...
    while (rb_pop(a->rb, &v)) {
        ...
        if (local_count % 10000 == 0) {
            printf("Consumer %d: consumed %d items so far\n",
                   a->consumer_index, local_count);
        }
    }
    printf("Consumer %d finished, consumed %lld items, sum = %lld\n",
...
int main(int argc, char** argv) {
    ...
    printf("Starting prodcons: P=%d C=%d N=%lld B=%d\n", P, C, N, B);
    ...
    printf("\nFINAL RESULTS\n");
    printf("Produced total: %lld items\n", produced_total);
    ...
    if (produced_total == consumed_total) {
        ...
        printf("ERROR: Produced (%lld) != Consumed (%lld)\n", 
