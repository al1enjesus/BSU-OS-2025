...
typedef enum { RACE_UNSYNC, RACE_MUTEX, RACE_ATOMIC } race_mode_t;
...
static _Atomic long long    shared_counter_atomic = 0;
...
static void* worker_unsync(void* arg) {
    ...
    for (long long i = 0; i < a->iterations_per_thread; i++) {
        long long tmp = shared_counter_unsync;
        for (volatile int k = 0; k < 100; k++) {
        }
        shared_counter_unsync = tmp + 1;
    }
    return NULL;
}
...
static race_mode_t parse_mode(const char* s) {
    if (strcmp(s, "unsync") == 0) return RACE_UNSYNC;
    if (strcmp(s, "mutex") == 0) return RACE_MUTEX;
    if (strcmp(s, "atomic") == 0) return RACE_ATOMIC;
...
int main(int argc, char** argv) {
    ...
    atomic_store_explicit(&shared_counter_atomic, 0, memory_order_relaxed);
    ...
    for (int i = 0; i < num_threads; i++) {
        ...
        switch (mode) {
        case RACE_UNSYNC: fn = worker_unsync; break;
        case RACE_MUTEX:  fn = worker_mutex;  break;
        case RACE_ATOMIC: fn = worker_atomic; break;
        }
    ...
    long long actual = 0;
    if (mode == RACE_UNSYNC)      actual = shared_counter_unsync;
    else if (mode == RACE_MUTEX)  actual = shared_counter_mutex;
    ...
    printf("mode=%s threads=%d iters_per_thread=%lld expected=%lld actual=%lld time_ms=%lld\n",
        (mode == RACE_UNSYNC ? "unsync" : mode == RACE_MUTEX ? "mutex" : "atomic"),
