...
int main(int argc, char** argv) {
    ...
    if (iters <= 0 || iters > 10000000) {
        fprintf(stderr, "Error: iterations must be between 1 and 10000000\n");
        return 1;
    }
    ...
    long long time_ms = end_ms - start_ms;
    double time_sec = time_ms / 1000.0;
    double throughput = (num_threads * iters) / time_sec;

    printf("Mode: %s, Threads: %d, Iterations: %lld\n", 
           (mode==MODE_UNSYNC?"unsync":mode==MODE_MUTEX?"mutex":"atomic"),
           num_threads, iters);
    printf("Expected: %lld, Actual: %lld\n", expected, actual);
    printf("Correct: %s\n", (actual == expected) ? "YES" : "NO");
    printf("Time: %lld ms (%.3f sec)\n", time_ms, time_sec);
