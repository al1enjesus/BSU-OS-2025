...
static void rb_push(ring_buffer_t* rb, int value) {
    sem_wait(&rb->empty);  // Ждём свободный слот
    ...
    sem_post(&rb->full);   // Сообщаем о новом занятом слоте
}

static int rb_pop(ring_buffer_t* rb, int* value) {
    // Пытаемся получить элемент с таймаутом для проверки завершения
    struct timespec ts;
    clock_gettime(CLOCK_REALTIME, &ts);
    ts.tv_sec += 1;  // Таймаут 1 секунда
    
    if (sem_timedwait(&rb->full, &ts) != 0) {
        // Таймаут - проверяем, есть ли ещё активные производители
        pthread_mutex_lock(&rb->mutex);
        int no_more_producers = (rb->producers_active == 0);
        pthread_mutex_unlock(&rb->mutex);
        
        if (no_more_producers) {
            return 0; // Больше элементов не будет
        }
        // Продолжаем ожидание
        return rb_pop(rb, value);
    }
    ...
    *value = rb->data[rb->head];
    ...
    sem_post(&rb->empty);  // Освобождаем слот
    return 1;
}
...
int main(int argc, char** argv) {
    ...
    printf("[prodcons] (semaphores) P=%d C=%d N=%lld B=%d produced=%lld consumed=%lld sum=%lld\n",
