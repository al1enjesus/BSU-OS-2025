...
// Изменяем mode_t на sync_mode_t чтобы избежать конфликта с системным типом
typedef enum { MODE_MUTEX, MODE_ATOMIC } sync_mode_t;
...
int main(int argc, char **argv) {
  if (argc < 4) {
    fprintf(stderr,
            "Usage: %s <num_threads> <iterations_per_thread> <mutex|atomic>\n",
            argv[0]);
    return 1;
  }
  ...
  printf("mode=%s threads=%d iters_per_thread=%lld expected=%lld actual=%lld "
         "time_ms=%lld\n",
         (mode == MODE_MUTEX ? "mutex" : "atomic"), num_threads, iters,
         expected, actual, (end_ms - start_ms));
