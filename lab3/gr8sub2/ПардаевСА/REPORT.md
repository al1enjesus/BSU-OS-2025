...
Дополнение:
Используется memory_order_relaxed потому что:
1. Операция инкремента независима от других операций с памятью
2. Не требует синхронизации с другими операциями чтения/записи
3. Обеспечивает атомарность без избыточных барьеров памяти
4. Максимальная производительность для простых счетчиков
