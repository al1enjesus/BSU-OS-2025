...
int main(int argc, char** argv) {
    if (argc < 4) {
        fprintf(stderr, "Usage: %s <num_threads> <iterations_per_thread> <mutex|atomic>\n", argv[0]);
        return 1;
    }
    ...
    if (iters < 0 || iters > 500000000)
    {
        fprintf(stderr, "Error: number of iterations must be between 1 and 500000000\n");
        return 1;
    }
    ...
    long long actual = 0;
    if (mode == MODE_MUTEX) actual = shared_counter_mutex;
