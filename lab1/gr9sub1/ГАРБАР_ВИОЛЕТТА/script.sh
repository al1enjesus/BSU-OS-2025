...
sudo grep -a "Invalid user" /var/log/auth.log 2>/dev/null | awk '{print $8}' | sort | uniq -c | sort -nr | head -n 5

# 2.2. Пользователи с неудачными попытками входа
echo -e "\n   Пользователи с неудачными попытками:"
sudo grep -a "Failed password" /var/log/auth.log 2>/dev/null | awk '{for(i=1;i<=NF;i++) if($i=="for") print $(i+1)}' | sort | uniq -c | sort -nr | head -n 5

# 2.3. Успешные входы
echo -e "\n   Успешные входы:"
sudo grep -a "session opened" /var/log/auth.log 2>/dev/null | grep "user" | awk '{print $11}' | sort | uniq -c | sort -nr | head -n 5
...
echo -e "\n3. СТАТИСТИКА УСПЕХОВ И ПРОВАЛОВ:"
...
ssh_ips=$(sudo grep -a "sshd.*from" /var/log/auth.log 2>/dev/null | grep -E -o '[0-9]+\.[0-9]+\.[0-9]+\.[0-9]+' | sort | uniq | wc -l)
