...
double benchmark_write_sync(const char* filename, size_t size, size_t buffer_size) {
    ...
    printf("Time: %.3f seconds\n", elapsed);
    printf("Throughput: %.2f MB/s\n", (size / (1024.0 * 1024.0)) / elapsed);
    printf("Note: O_SYNC forces each write to wait for physical disk write.\n");
...
void benchmark_all_methods(size_t file_size_mb) {
    ...
    times[2] = benchmark_write_sync("test_write_sync.bin", file_size, optimal_buffer);
    unlink("test_write_sync.bin");
    sleep(1);

    times[3] = benchmark_mmap("test_mmap.bin", file_size);
    unlink("test_mmap.bin");

    printf("\n=== Summary ===\n");
    printf("Method            | Time (s) | Throughput (MB/s)\n");
    printf("------------------|----------|------------------\n");
    printf("fwrite            | %7.3f  | %7.2f\n", times[0], (file_size / (1024.0 * 1024.0)) / times[0]);
    printf("write             | %7.3f  | %7.2f\n", times[1], (file_size / (1024.0 * 1024.0)) / times[1]);
    printf("write (O_SYNC)    | %7.3f  | %7.2f\n", times[2], (file_size / (1024.0 * 1024.0)) / times[2]);
    printf("mmap              | %7.3f  | %7.2f\n", times[3], (file_size / (1024.0 * 1024.0)) / times[3]);

    int fastest = 0;
    for (int i = 1; i < 4; i++) {
        if (times[i] < times[fastest] && times[i] > 0) {
            fastest = i;
        }
    }

    const char* methods[] = { "fwrite", "write", "write with O_SYNC", "mmap" };
    printf("\nFastest method: %s\n", methods[fastest]);

    printf("\nFactors affecting performance:\n");
    printf("- stdio (fwrite) has user-space buffering\n");
    printf("- write() goes directly to kernel, but still uses page cache\n");
    printf("- write(O_SYNC) waits for physical disk write (very slow!)\n");
