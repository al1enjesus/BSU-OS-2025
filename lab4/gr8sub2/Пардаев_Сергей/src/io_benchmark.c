...
static double benchmark_fwrite(const char *filename, size_t size,
                               size_t buffer_size) {
...
static double benchmark_write(const char *filename, size_t size,
                              size_t buffer_size) {
...
static double benchmark_write_sync(const char *filename, size_t size,
                                   size_t buffer_size) {
...
static void benchmark_all_methods(size_t file_size_mb) {
  ...
  printf("- stdio (fwrite) сглаживает мелкие записи => часто быстрее при "
         "мелких блоках\n");
    ...
    if (fd < 0) {
      ...
      if (p == MAP_FAILED) {
        ...
        printf("Time: %.3f s, Speed: %.2f MB/s (checksum=%llu)\n", dt,
               (file_size / 1024.0 / 1024.0) / dt, sum);
        munmap(p, file_size);
        close(fd);
      }
    }
...
int main(int argc, char *argv[]) {
  ...
  for (int i = 1; i < argc; i++) {
    if (strcmp(argv[i], "--size") == 0 && i + 1 < argc) {
      ...
      printf("\nOptions:\n");
      printf(
          "  --size SIZE_MB   Size of test file in megabytes (default: %d)\n",
          DEFAULT_SIZE_MB);
      printf("\nExamples:\n");
