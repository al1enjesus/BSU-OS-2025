...
static const char *color_for_path(const MemorySegment *s) {
  ...
  if (strstr(p, "[stack"))
    return C_STACK;
  if (strstr(p, "[vdso]") || strstr(p, "[vvar]"))
    return C_VSYS;
  if (strstr(p, ".so"))
    return C_LIB;
  if (s->perms[2] == 'x')
...
static int read_memory_metrics(pid_t pid, MemoryMetrics *m) {
  ...
  while (fgets(line, sizeof(line), f)) {
    if (sscanf(line, "VmSize: %lu kB", &m->vm_size) == 1)
      continue;
    if (sscanf(line, "VmRSS: %lu kB", &m->vm_rss) == 1)
      continue;
    if (sscanf(line, "VmData: %lu kB", &m->vm_data) == 1)
      continue;
    if (sscanf(line, "VmStk: %lu kB", &m->vm_stk) == 1)
      continue;
    if (sscanf(line, "VmExe: %lu kB", &m->vm_exe) == 1)
      continue;
    if (sscanf(line, "VmLib: %lu kB", &m->vm_lib) == 1)
      continue;
  }
  fclose(f);
  return 0;
...
static int read_pss_from_smaps(pid_t pid, MemoryMetrics *m) {
  ...
  while (fgets(line, sizeof(line), f)) {
    ...
    if (sscanf(line, "Shared_Clean: %lu kB", &v) == 1) {
      sc += v;
      continue;
    }
    if (sscanf(line, "Shared_Dirty: %lu kB", &v) == 1) {
      sd += v;
      continue;
    }
...
static int read_pss(pid_t pid, MemoryMetrics *m) {
  ...
  if (f) {
    char line[256];
    while (fgets(line, sizeof(line), f)) {
      if (sscanf(line, "Pss: %lu kB", &m->pss) == 1)
        continue;
      if (sscanf(line, "Shared_Clean: %lu kB", &m->shared_clean) == 1)
        continue;
      if (sscanf(line, "Shared_Dirty: %lu kB", &m->shared_dirty) == 1)
        continue;
      if (sscanf(line, "Private_Clean: %lu kB", &m->private_clean) == 1)
        continue;
      if (sscanf(line, "Private_Dirty: %lu kB", &m->private_dirty) == 1)
        continue;
    }
    fclose(f);
    return 0;
...
static void summarize_map(pid_t pid) {
  ...
  int cnt = 0;
  if (read_memory_map(pid, &segs, &cnt) != 0)
    return;

  unsigned long kb_heap = 0, kb_stack = 0, kb_vdso = 0, kb_vvar = 0,
                kb_anon = 0, kb_libs = 0, kb_exe = 0, kb_other = 0;
  ...
  for (int i = 0; i < cnt; i++) {
    ...
    if (strstr(p, "[heap]"))
      kb_heap += sz_kb;
    else if (strstr(p, "[stack"))
      kb_stack += sz_kb;
    else if (strstr(p, "[vdso]"))
      kb_vdso += sz_kb;
    else if (strstr(p, "[vvar]"))
      kb_vvar += sz_kb;
    else if (p[0] == 0 || p[0] == '[')
      kb_anon += sz_kb;
    else if (strstr(p, ".so"))
      kb_libs += sz_kb;
    else if (segs[i].perms[2] == 'x')
      kb_exe += sz_kb;
    else
      kb_other += sz_kb;
  }
...
static FILE *open_csv(const char *path) {
  ...
  fseek(f, 0, SEEK_END);
  if (ftell(f) == 0)
    fprintf(f, "timestamp,pid,VSZ_kB,RSS_kB,PSS_kB,USS_kB,minor,major\n");
  return f;
}
  ...
  while (1) {
    ...
    if (csv) {
      ...
      fprintf(csv, "%ld,%d,%lu,%lu,%lu,%lu,%lu,%lu\n", (long)now, (int)pid,
              m.vm_size, m.vm_rss, m.pss, uss, (unsigned long)pf.minor_faults,
              (unsigned long)pf.major_faults);
      fflush(csv);
    }
...
static void compare_processes(pid_t p1, pid_t p2) {
  ...
  struct Row {
    ...
    unsigned long y;
      ...
      {"VSZ (KB)", a.vm_size, b.vm_size},  {"RSS (KB)", a.vm_rss, b.vm_rss},
      {"PSS (KB)", a.pss, b.pss},          {"USS (KB)", a_uss, b_uss},
      {"Heap (KB)", a.vm_data, b.vm_data}, {"Stack (KB)", a.vm_stk, b.vm_stk},
...
static void analyze_shared_libs(pid_t pid, int top_n) {
  ...
  for (int i = 0; i < cnt; i++) {
    if (!strstr(segs[i].path, ".so"))
      continue;
    if (!segs[i].path[0])
      continue;
    int idx = -1;
    for (int j = 0; j < nlibs; j++) {
      if (strcmp(libs[j].path, segs[i].path) == 0) {
        idx = j;
        break;
      }
    }
  ...
  for (int i = 0; i < nlibs; i++) {
    ...
    for (int j = i + 1; j < nlibs; j++) {
      if (libs[j].share_count > libs[best].share_count ||
          (libs[j].share_count == libs[best].share_count &&
           libs[j].self_kb > libs[best].self_kb)) {
        best = j;
      }
    }
    if (best != i) {
...
int main(int argc, char *argv[]) {
  ...
  int compare_mode = 0, show_map = 0, want_libs = 0, interactive = 0,
      no_color = 0;
  pid_t pid2 = 0;
