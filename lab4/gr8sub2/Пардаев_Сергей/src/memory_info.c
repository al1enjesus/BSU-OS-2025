...
static void print_memory_metrics(pid_t pid) {
  ...
  while (fgets(line, sizeof(line), f)) {
    if (sscanf(line, "VmSize: %lu kB", &vm_size) == 1)
      continue;
    if (sscanf(line, "VmRSS: %lu kB", &vm_rss) == 1)
      continue;
    if (sscanf(line, "VmData: %lu kB", &vm_data) == 1)
      continue;
    if (sscanf(line, "VmStk: %lu kB", &vm_stk) == 1)
      continue;
  }
  fclose(f);
...
static void demonstrate_memory_types(void) {
  ...
  size_t pg = (size_t)sysconf(_SC_PAGESIZE);
  for (size_t i = 0; i < heap_size; i += pg)
    heap_var[i] = 1;
  ...
  if (mmap_size > 0) {
    ...
    if (mmap_var == MAP_FAILED) {
      ...
      for (size_t i = 0; i < mmap_size; i += pg)
        ((char *)mmap_var)[i] = 2;
  ...
  free(heap_var);
  if (mmap_var && mmap_var != MAP_FAILED && mmap_size > 0)
...
int main(int argc, char *argv[]) {
  if (argc == 2) {
    ...
    printf("This will allocate different types of memory and show the "
           "results.\n\n");
