...
void print_memory_map(pid_t pid) {
    ...
    while (fgets(line, sizeof(line), f)) {
        ...
        if (sscanf(line, "%lx-%lx %4s %*s %*s %*s %255[^\n]", 
            ...
            if (size_kb < 1024) {
                printf("%08lx-%08lx %-6s %4lu KB   %s\n", start, end, perms, size_kb, path_str);
            } else {
                printf("%08lx-%08lx %-6s %4lu MB   %s\n", start, end, perms, size_kb/1024, path_str);
            }
        }
    }
...
void demonstrate_memory_types() {
    ...
    // ПОКАЗАТЬ МЕТРИКИ ДО ВЫДЕЛЕНИЯ ПАМЯТИ
    ...
    printf("   Touching heap memory to trigger page faults...");
    ...
    printf("   Touching mmap memory to trigger page faults...");
    memset(mmap_var, 'M', mmap_size);
    printf(" mmap touch complete.\n");

    // ПОКАЗАТЬ МЕТРИКИ ПОСЛЕ ВЫДЕЛЕНИЯ ПАМЯТИ
