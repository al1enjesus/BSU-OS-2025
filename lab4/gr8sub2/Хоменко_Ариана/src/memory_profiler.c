...
// Структура для хранения метрик памяти процесса
typedef struct {
...
// Структура для статистики page faults
typedef struct {
    unsigned long minor_faults;  // Мягкие ошибки страниц
    unsigned long major_faults;  // Жёсткие ошибки страниц
} PageFaults;

// Структура для описания сегмента памяти
typedef struct {
    unsigned long start;  // Начальный адрес сегмента
    unsigned long end;    // Конечный адрес сегмента
    char perms[5];        // Права доступа (rwxp)
    char path[256];       // Путь к файлу или тип сегмента
} MemorySegment;

/*
 * Чтение основных метрик памяти из /proc/[PID]/status
...
int read_memory_metrics(pid_t pid, MemoryMetrics *metrics) {
    ...
    char line[256];
    // Читаем файл построчно и парсим нужные метрики
...
/*
 * Чтение расширенных метрик из /proc/[PID]/smaps_rollup
 * Включает PSS, USS и детализацию shared/private памяти
 */
...
/*
 * Чтение статистики page faults из /proc/[PID]/stat
 * Формат файла сложный, требуется специальный парсинг
 */
int read_page_faults(pid_t pid, PageFaults *faults) {
    ...
    // Ищем закрывающую скобку команды (команда может содержать пробелы)
    char *p = strrchr(line, ')');
    if (!p) {
        fclose(f);
        return -1;
    ...
    // Парсим поля после команды (minflt и majflt)
...
/*
 * Чтение карты памяти процесса из /proc/[PID]/maps
 * Возвращает массив сегментов и их количество
 */
int read_memory_map(pid_t pid, MemorySegment **segments, int *count) {
    ...
    // Сначала подсчитываем количество строк (сегментов)
    char line[512];
    int lines = 0;
    while (fgets(line, sizeof(line), f)) {
        lines++;
    ...
    *count = lines;
    ...
    // Выделяем память под все сегменты
    *segments = malloc(*count * sizeof(MemorySegment));
    ...
    // Читаем файл заново и парсим каждый сегмент
    rewind(f);
    
    int i = 0;
    while (fgets(line, sizeof(line), f) && i < *count) {
        MemorySegment *seg = &(*segments)[i];
        seg->path[0] = '\0';  // Инициализируем путь
        
        // Парсим строку формата: start-end perms offset dev inode path
...
/*
 * Форматированный вывод размера в человеко-читаемом виде
 * Автоматически выбирает единицы измерения (KB, MB, GB)
 */
...
/*
 * Основная функция вывода информации о процессе
 * Показывает все основные метрики памяти и статистику
 */
void print_process_info(pid_t pid) {
    ...
    // Чтение и вывод основных метрик памяти
    ...
    if (read_memory_metrics(pid, &metrics) == 0) {
        ...
        // Пытаемся получить расширенные метрики (PSS, USS)
...
/*
 * Вывод сокращённой карты памяти (первые 20 сегментов)
 */
void print_memory_map_summary(pid_t pid) {
    ...
    // Выводим первые 20 сегментов или все, если их меньше
...
/*
 * Режим реального времени - мониторинг процесса с заданным интервалом
 * Показывает изменения метрик между обновлениями
 */
void watch_process(pid_t pid, int interval) {
    ...
    while (1) {
        // Очищаем экран (ANSI escape codes)
        printf("\033[2J\033[H");
        ...
        // Page faults с дельтой за интервал
...
/*
 * Вспомогательная функция для сравнения процессов
 * Выводит строку сравнения для одной метрики
 */
...
/*
 * Сравнение двух процессов - вывод таблицы с метриками
 */
...
/*
 * Главная функция - парсинг аргументов и запуск соответствующих режимов
 */
int main(int argc, char *argv[]) {
    ...
    if (compare_mode) {
        ...
        // Обычный режим - однократный вывод информации
