...
void print_memory_metrics(pid_t pid) {
    ...
    while (fgets(line, sizeof(line), f)) {
        ...
        {
            result += sscanf(line, "VmSize:%*[\t ]%lu kB", &vm_size);
        }
        else if (strncmp(line, "VmRSS:", 6) == 0)
        {
            result += sscanf(line, "VmRSS:%*[\t ]%lu kB", &vm_rss);
        }
        else if (strncmp(line, "VmData:", 7) == 0)
        {
            result += sscanf(line, "VmData:%*[\t ]%lu kB", &vm_data);
        }
        else if (strncmp(line, "VmStk:", 6) == 0)
        {
            result += sscanf(line, "VmStk:%*[\t ]%lu kB", &vm_stk);
        } 
    }
...
void print_memory_map(pid_t pid) {
    ...
    while (fgets(line, sizeof(line), f)) {
        ...
        // Распарсить строку
        result = sscanf(line, "%lx-%lx %s %*s %*s %*s %[^\n]", &start, &end, perms, path_str);
        if (result != 4)
        {
            continue;
        }
...
void demonstrate_memory_types() {
    ...
    // printf("   Filling heap memory to trigger page faults...\n");
    for (size_t i = 0; i < heap_size; i += 4096) {
        heap_var[i] = 'H';  // Записываем в каждую страницу
    }
    ...
    // printf("   Filling anonymous mmap memory to trigger page faults...\n");
    for (size_t i = 0; i < mmap_size; i += 4096) {
        *((char*)mmap_var + i) = 'M';  // Записываем в каждую страницу
    }
