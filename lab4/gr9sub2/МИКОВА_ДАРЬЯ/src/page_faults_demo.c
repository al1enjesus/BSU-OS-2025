...
void demo_full_write() {
    printf("\n=== Demo 3: Full memory write (memset) ===\n");
    ...
    size_t expected_faults = size / PAGE_SIZE;
    printf("Expected page faults: ~%zu\n", expected_faults);
...
void demo_random_access() {
    ...
    for (int i = 0; i < num_accesses; i++) {
        // Выбираем случайную страницу и пишем в ее начало
        size_t random_page_index = rand() % num_pages;
    ...
    printf("Note: Max faults possible is %d (if all accesses unique)\n", num_accesses);
    printf("      (Actual faults will be <= %d)\n", num_accesses);
...
void demo_rereading() {
    ...
    printf("Note: ~0 new page faults expected (pages already in memory).\n");
...
void demo_calloc_vs_malloc() {
    ...
    // malloc + memset
    printf("Method 1: malloc + memset(0)\n");
    long start_minor, start_major;
    get_page_faults(&start_minor, &start_major);
    ...
    if (ptr2) {
        ...
        // Чтение: calloc может использовать zero page optimization
        // (все страницы изначально указывают на одну zero-page в read-only)
        long read_start_minor, read_start_major;
        get_page_faults(&read_start_minor, &read_start_major);

        volatile char tmp;
        for (size_t i = 0; i < size; i += PAGE_SIZE) {
            tmp = ptr2[i];  // Чтение
        }
        (void)tmp; // Избежать warning

        print_page_fault_delta("After calloc + read", read_start_minor, read_start_major);
        printf("Note: Reading from calloc'd memory causes minor faults (mapping zero page).\n");


        // Теперь запишем в память
        long write_start_minor, write_start_major;
        get_page_faults(&write_start_minor, &write_start_major);

        for (size_t i = 0; i < size; i += PAGE_SIZE) {
            ptr2[i] = 1;  // Запись (вызовет copy-on-write)
        }

        print_page_fault_delta("After calloc + write", write_start_minor, write_start_major);
        printf("Note: Writing to calloc'd memory causes CoW faults.\n");
...
int main() {
    printf("Page Faults Demonstration (Variant 1, Task C)\n");
    printf("===========================================\n");
    ...
    printf("5. calloc() uses optimizations (zero page, CoW)\n");
