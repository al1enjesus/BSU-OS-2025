...
find_bash_pid() {
    local pid=$(pgrep -o bash 2>/dev/null)
    if [ -z "$pid" ] || [ "$pid" = "0" ]; then
        echo ""
    else
        echo "$pid"
...
BASH_PID=$(find_bash_pid)
if [ -n "$BASH_PID" ]; then
    echo "Найден процесс bash с PID: $BASH_PID" | tee -a logs/task_a.log
    ./bin/memory_analysis $BASH_PID | tee -a logs/task_a.log
else
    echo -e "${YELLOW}ВНИМАНИЕ: Процесс bash не найден${NC}" | tee -a logs/task_a.log
    echo "Анализируем процесс init (PID 1) вместо bash" | tee -a logs/task_a.log
    ./bin/memory_analysis 1 | tee -a logs/task_a.log
fi
...
echo ""
echo -e "${YELLOW}Для включения в отчёт:${NC}"
echo "  Скопируйте содержимое из logs/ в REPORT.MD"
