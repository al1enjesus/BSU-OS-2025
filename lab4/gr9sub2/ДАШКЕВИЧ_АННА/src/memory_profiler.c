...
static int read_smaps_rollup_pid(pid_t pid, unsigned long* pss_kb, unsigned long* uss_kb) {
    ...
    if (!f) return -1;
    char line[512];
    int seen_pss = 0, seen_pclean = 0, seen_pdirty = 0;
...
static int read_stat_faults_pid(pid_t pid, unsigned long* minflt, unsigned long* majflt, char* comm, size_t commsz) {
    ...
    const char *rest = rpar + 2;
    char st;
    unsigned long ppid, pgrp, session, tty_nr, tpgid, flags, cminflt, cmaj;
    int n = sscanf(rest, "%c %lu %lu %lu %lu %lu %lu %lu %lu %lu %lu",
...
static int collect_stats(pid_t pid, memstats_t* ms, char* comm, size_t commsz) {
    ...
    (void)read_status_kb_pid(pid, "VmRSS:", &ms->rss_kb);
    (void)read_smaps_rollup_pid(pid, &ms->pss_kb, &ms->uss_kb);

    unsigned long minf = 0, majf = 0;
    (void)read_stat_faults_pid(pid, &minf, &majf, comm, commsz);
    ms->minor_flts = minf;
    ms->major_flts = majf;

    if (ms->uss_kb && ms->rss_kb && ms->rss_kb >= ms->uss_kb) {
        ms->priv_kb = ms->uss_kb;
        ms->shared_kb = ms->rss_kb - ms->uss_kb;
    }
    return 0;
}
...
static void print_stats(pid_t pid, const memstats_t* ms, const memstats_t* prev, const char* comm) {
    ...
    if (prev) {
        printf("  (Δ ");
        long d = (long)ms->rss_kb - (long)prev->rss_kb;
        print_human((unsigned long)(d >= 0 ? d : -d) * 1024UL);
        printf(d >= 0 ? "↑)\n" : "↓)\n");
    } else {
        printf("\n");
    }
    ...
    printf("Shared: ");
    print_human(ms->shared_kb * 1024UL);
    printf("\n");

    printf("Private: ");
    print_human(ms->priv_kb * 1024UL);
    printf("\n");

    printf("Page Faults: Minor %lu", ms->minor_flts);
    if (prev) {
        long d = (long)ms->minor_flts - (long)prev->minor_flts;
        printf(" (Δ %ld)", d);
    }
    printf(", Major %lu", ms->major_flts);
    if (prev) {
        long d = (long)ms->major_flts - (long)prev->major_flts;
        printf(" (Δ %ld)", d);
    }
    printf("\n");
}
...
int main(int argc, char** argv) {
    ...
    if (strcmp(argv[1], "--watch") == 0) {
        if (argc < 3 || !is_positive_number(argv[2])) {
            fprintf(stderr, "Error: --watch requires a positive PID.\n");
            print_usage(argv[0]);
            return 1;
        }
        watch = 1;
    ...
    char procdir[64];
    snprintf(procdir, sizeof(procdir), "/proc/%d", pid1);
    if (access(procdir, F_OK) != 0) {
        fprintf(stderr, "Error: Process %d does not exist or /proc is not accessible.\n", pid1);
        return 2;
    }
    if (compare) {
        snprintf(procdir, sizeof(procdir), "/proc/%d", pid2);
        if (access(procdir, F_OK) != 0) {
            fprintf(stderr, "Error: Process %d does not exist or /proc is not accessible.\n", pid2);
            return 2;
        }
    ...
    if (compare) {
        ...
        if (collect_stats(pid1, &a, ca, sizeof(ca)) < 0 || collect_stats(pid2, &b, cb, sizeof(cb)) < 0) {
            fprintf(stderr, "Failed to read stats (insufficient permissions or process ended)\n");
            return 3;
        }
    ...
    if (!watch) {
        ...
        if (collect_stats(pid1, &ms, comm, sizeof(comm)) < 0) {
            fprintf(stderr, "Failed to read stats (insufficient permissions or process ended)\n");
            return 3;
        }
        ...
        int first = 1;
        while (1) {
            if (collect_stats(pid1, &cur, comm, sizeof(comm)) < 0) {
