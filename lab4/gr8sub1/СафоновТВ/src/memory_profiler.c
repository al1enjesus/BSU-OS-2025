...
int read_pss(pid_t pid, MemoryMetrics *metrics) {
    ...
    if (!f) {
        ...
        while (fgets(line, sizeof(line), f)) {
            if (sscanf(line, "Pss: %lu kB", &total_pss) == 1) {
                metrics->pss += total_pss;
            }
            if (sscanf(line, "Shared_Clean: %lu kB", &total_shared_clean) == 1) {
                metrics->shared_clean += total_shared_clean;
            }
            if (sscanf(line, "Shared_Dirty: %lu kB", &total_shared_dirty) == 1) {
                metrics->shared_dirty += total_shared_dirty;
            }
            if (sscanf(line, "Private_Clean: %lu kB", &total_private_clean) == 1) {
                metrics->private_clean += total_private_clean;
            }
            if (sscanf(line, "Private_Dirty: %lu kB", &total_private_dirty) == 1) {
...
void print_memory_map_summary(pid_t pid) {
    ...
    int heap_count = 0, stack_count = 0, lib_count = 0, anon_count = 0, other_count = 0;
    unsigned long heap_size = 0, stack_size = 0, lib_size = 0, anon_size = 0, other_size = 0;
    ...
    for (int i = 0; i < count && i < 20; i++) {
        ...
        if (strstr(seg->path, "[heap]")) {
            ...
            lib_count++;
            lib_size += size_kb;
        } else if (strcmp(seg->path, "(anonymous)") == 0 || seg->path[0] == '\0') {
            anon_count++;
            anon_size += size_kb;
        } else {
            other_count++;
    ...
    printf("\nMemory Map Summary:\n");
    printf("  Heap:      %2d segments, ", heap_count); print_size(heap_size); printf("\n");
    printf("  Stack:     %2d segments, ", stack_count); print_size(stack_size); printf("\n");
    printf("  Libraries: %2d segments, ", lib_count); print_size(lib_size); printf("\n");
    printf("  Anonymous: %2d segments, ", anon_count); print_size(anon_size); printf("\n");
    printf("  Other:     %2d segments, ", other_count); print_size(other_size); printf("\n");
...
void compare_processes(pid_t pid1, pid_t pid2) {
    ...
    print_size((long)m1.vm_size - (long)m2.vm_size); printf("\n");

    printf("%-20s ", "RSS"); 
    print_size(m1.vm_rss); printf(" "); 
    print_size(m2.vm_rss); printf(" ");
    print_size((long)m1.vm_rss - (long)m2.vm_rss); printf("\n");

    if (m1.pss > 0 && m2.pss > 0) {
        printf("%-20s ", "PSS"); 
        print_size(m1.pss); printf(" "); 
        print_size(m2.pss); printf(" ");
        print_size((long)m1.pss - (long)m2.pss); printf("\n");
    }

    printf("%-20s ", "Data+Heap"); 
    print_size(m1.vm_data); printf(" "); 
    print_size(m2.vm_data); printf(" ");
    print_size((long)m1.vm_data - (long)m2.vm_data); printf("\n");

    printf("%-20s ", "Stack"); 
    print_size(m1.vm_stk); printf(" "); 
    print_size(m2.vm_stk); printf(" ");
    print_size((long)m1.vm_stk - (long)m2.vm_stk); printf("\n");

    PageFaults f1, f2;
    if (read_page_faults(pid1, &f1) == 0 && read_page_faults(pid2, &f2) == 0) {
        printf("\nPage Faults:\n");
        printf("  Minor: %10lu %15lu %15ld\n", f1.minor_faults, f2.minor_faults, (long)f1.minor_faults - f2.minor_faults);
        printf("  Major: %10lu %15lu %15ld\n", f1.major_faults, f2.major_faults, (long)f1.major_faults - f2.major_faults);
    }
}
