...
void demonstrate_memory_types() {
    ...
    memset(heap_var, 'H', heap_size);  // Вызываем page faults
    printf("2. Heap allocated: 2 MB at %p\n", (void*)heap_var);
    ...
    memset(mmap_var, 'M', mmap_size);  // Вызываем page faults
    printf("3. Anonymous mmap: 5 MB at %p\n", mmap_var);

    printf("\nMemory allocated. Check memory metrics:\n");
