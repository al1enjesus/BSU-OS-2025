...
unsigned long long read_with_syscalls(const char *filename) {
    ...
    printf("Throughput: %.2f MB/s\n", (sb.st_size / (1024.0 * 1024.0)) / (end_time - start_time));
...
int main(int argc, char *argv[]) {
    ...
    printf("Note: For accurate results, clear page cache with:\n");
    ...
    if (sum1 == sum2 && sum2 == sum3) {
        ...
        printf("  read(): %llu\n", sum1);
        printf("  mmap(): %llu\n", sum2);
        printf("  mmap+sequential: %llu\n", sum3);
    }

    printf("\n=== Summary ===\n");
    printf("Method comparison completed.\n");
    printf("Typical observations:\n");
    printf("- mmap() is faster for large files due to fewer system calls\n");
    printf("- read() has more predictable memory usage\n");
    printf("- madvise(SEQUENTIAL) can improve performance for sequential access\n");
