...
// Функция для получения текущих page faults
...
// Функция для вывода разницы в page faults
...
void demo_random_access() {
    ...
    int pages_touched = 0;
    int *accessed_pages = calloc(size / PAGE_SIZE, sizeof(int));

    for (int i = 0; i < num_accesses; i++) {
        size_t random_offset = (rand() % (size / PAGE_SIZE)) * PAGE_SIZE;
        ptr[random_offset] = 'C';
        
        if (!accessed_pages[random_offset / PAGE_SIZE]) {
            pages_touched++;
            accessed_pages[random_offset / PAGE_SIZE] = 1;
        }
    }
...
int main() {
    ...
    printf("5. calloc() may use zero page optimization (CoW)\n");
    printf("6. Random access causes more page faults than sequential\n");
