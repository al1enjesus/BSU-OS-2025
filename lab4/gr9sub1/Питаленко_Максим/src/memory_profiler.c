...
int read_page_faults(pid_t pid, PageFaults *faults) {
    ...
    // Читаем всю строку и парсим вручную
    char line[1024];
    if (fgets(line, sizeof(line), f)) {
        ...
        while (token != NULL) {
            if (field == 10) {  // minflt
                faults->minor_faults = strtoul(token, NULL, 10);
            } else if (field == 12) {  // majflt
                faults->major_faults = strtoul(token, NULL, 10);
                break;
            }
            token = strtok(NULL, " ");
            field++;
...
void print_memory_map_summary(pid_t pid) {
    ...
    for (int i = 0; i < count && i < 20; i++) {
        ...
        if (strstr(seg->path, "[heap]")) heap_count++;
        else if (strstr(seg->path, "[stack]")) stack_count++;
        else if (strstr(seg->path, ".so")) lib_count++;
        else if (strcmp(seg->path, "(anonymous)") == 0) anon_count++;
        else other_count++;
    }
...
void compare_processes(pid_t pid1, pid_t pid2) {
    ...
    if (m1.pss > 0 && m2.pss > 0) {
        ...
        printf("%-20s ", "USS"); print_size(uss1); printf(" "); print_size(uss2); printf("\n");
    }
}
