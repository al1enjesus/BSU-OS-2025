...
void demo_random_access() {
    ...
    for (int i = 0; i < num_accesses; i++) {
        size_t rand_offset = (size_t)((double)rand() / RAND_MAX * size);
        rand_offset -= rand_offset % PAGE_SIZE;  // Выровнять по странице
        ptr[rand_offset] = 'R';
    }
...
void demo_calloc_vs_malloc() {
    ...
    if (ptr2) {
        // Чтение (может не вызвать faults из-за zero page)
        for (size_t i = 0; i < size; i += PAGE_SIZE) {
